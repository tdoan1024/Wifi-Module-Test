...
#include "RST.h"
...
#if defined(RST__PC)
    #define RST_SetP4PinDriveMode(shift, mode)  \
    do { \
        RST_PC =   (RST_PC & \
                                (uint32)(~(uint32)(RST_DRIVE_MODE_IND_MASK << \
                                (RST_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (RST_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define RST_SetP4PinDriveMode(shift, mode)  \
        do { \
            RST_USBIO_CTRL_REG = (RST_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(RST_DRIVE_MODE_IND_MASK << \
                                    (RST_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (RST_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		RST_SetP4PinDriveMode(RST__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(RST_DR & (uint8)(~RST_MASK));
    drVal = (drVal | ((uint8)(value << RST_SHIFT) & RST_MASK));
    RST_DR = (uint32)drVal;
}
...
uint8 RST_Read(void)
{
    return (uint8)((RST_PS & RST_MASK) >> RST_SHIFT);
}
...
{
    return (uint8)((RST_DR & RST_MASK) >> RST_SHIFT);
}
...
void RST_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  RST_INTCFG & (uint32)(~(uint32)position);
    RST_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(RST_INTSTAT & RST_MASK);
	RST_INTSTAT = maskedStatus;
    return maskedStatus >> RST_SHIFT;
}
